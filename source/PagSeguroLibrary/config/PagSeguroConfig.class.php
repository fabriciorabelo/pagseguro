...
class PagSeguroConfig
{
    ...
    public static function setConfig($key, $environment, $value = null)
    {
        if (isset(self::$data['credentials']) &&
            isset(self::$data['credentials'][$key][$environment])
        ) {
            self::$data['credentials'][$key][$environment] = $value;
        } else {
            throw new Exception("Config \$data['credentials'][{$key}][{$environment}] not set.");
        }
    }
    ...
    public static function setEmail($value)
    {
        if (isset(self::$data['credentials']) &&
            ...
            self::$data['credentials']['email'] = $value;
        } else {
